	...
	public List<SubjectBean> getSubsBySpec(String specid,String tdept,String tname,String substatus) throws Exception {
		...
		if(substatus.equals("")) {
			...
			while (it.hasNext()) {
				...
				if(tempstr.length()>=8 && tempstr.substring(0, 8).equals("已提交-等待审核")) tempstr=tempstr.substring(0, 8);
...
//	批量发布课题
	...
	public void validateSubBeforeSubmit(String subid) throws Exception{
		...
		for(int i=0;i<subject.getProgress().size();i++){
			...
			if(prog.getContent().equals("")||prog.getStartenddate().equals("")) throw new Exception("课题进程表中的每一项都不能为空！");
		}
		////////////////////////////
